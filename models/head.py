...
class iBOTHead(DINOHead):
    ...
    def __init__(self, *args, patch_out_dim=8192, norm=None, act='gelu', last_norm=None, 
                 ...
                 shared_head=False, use_middle_cls_token=False, **kwargs):
    ...
    def forward(self, x):
        ...
        if self.last_layer is not None:
            ...
            if self.use_middle_cls_token:
                M = x.shape[1] // 2
                x1 = self.last_layer(x[:, M])
                x2 = self.last_layer2(torch.cat((x[:, :M], x[:, M+1:]), dim=1))
            else:
            ...
            if self.use_middle_cls_token:
                ...
                x1 = self.mlp[-1](x[:, 0])
                x2 = self.mlp2(torch.cat((x[:, :M], x[:, M+1:]), dim=1))
